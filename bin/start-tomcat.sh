...
echo "CATALINA_OPTS=\"-Dadditional.kfs.config.locations=/infra/conf/kfs-config.properties -Djava.awt.headless=true -Dco.unpackwars=true -Dcatalina.logs=/infra/logs/tomcat -Xdebug -Xrunjdwp:transport=dt_socket,address=localhost:50101,server=y,suspend=n -Dcom.sun.jndi.ldap.read.timeout=60000 -Dcom.sun.jndi.ldap.connect.pool.timeout=30000 -Dcom.sun.jndi.ldap.connect.timeout=10000 \"" >> /etc/default/tomcat7
